...
exports.bloqueo_cuenta = async (req, res) => {    
    const filePath = path.join(__dirname, 'utils', 'bloqueo_cuenta.json');
...
exports.razon = async (req, res) => {    
    const filePath = path.join(__dirname, 'utils', 'razon.json');
    console.log('File Path:', filePath);    
    fs.readFile(filePath, 'utf8', (error, data) => {
        if (error) {
            return res.status(500).json({
