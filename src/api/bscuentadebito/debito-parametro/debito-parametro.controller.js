...
exports.debito_parametro = async (req, res) => {
    const filePath = path.join(__dirname, 'utils', 'debito_parametro.json');
    console.log('File Path:', filePath);
    fs.readFile(filePath, 'utf8', (error, data) => {
        if (error) {
            return res.status(500).json({
...
exports.tabla_distintos = async (req, res) => {
    const filePath = path.join(__dirname, 'utils', 'tabla_distintos.json');
    console.log('File Path:', filePath);
    fs.readFile(filePath, 'utf8', (error, data) => {
        if (error) {
            return res.status(500).json({
...
exports.obtener_parametro_id = async (req, res) => {
    const filePath = path.join(__dirname, 'utils', 'obtener_parametro_id.json');
    console.log('File Path:', filePath);
    fs.readFile(filePath, 'utf8', (error, data) => {
        if (error) {
            return res.status(500).json({
